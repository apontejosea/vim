Graph_PercentFlowVsTime6Lines <-  function(from, to, data=tcd2)
{ 
  library(lattice)
  options(nsmall=2)
  DPM             <-  matrix(
  ...
  myFilter        <-  with(data, Lines %in% c(6) & format(Dates, '%Y') >= 2008 & Outliers==FALSE)
  Q54             <-  with(data[myFilter,], mean(E3+W3))
  ...
  P               <-  data[myFilter, lineNames]/data$RECQ[myFilter]
