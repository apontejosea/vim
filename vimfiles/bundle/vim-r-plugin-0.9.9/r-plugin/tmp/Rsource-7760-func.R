Graph_Predict <- function(field, lines.open = 4, data, 
  ...
  fit.filter <-  with(data, Lines == lines.open & Outliers == FALSE & 
                      Dates >= fit.range[1] &
                      Dates <= fit.range[2])

  pred.filter<-  with(data, Lines == lines.open & Outliers == FALSE & 
                      Dates >= pred.range[1] &
                      Dates <= pred.range[2])
  ...
  if(sum(fit.filter)<=0) {
    ...
    modelFit      <-  lm(eval(modl), data=data[fit.filter,])
    ...
    txt.all       <- data.frame(txt =paste(txt.formula, txt.rsq, sep='\n'), x =
                                as.Date(fit.dt[1]), y = 20)

    p             <- p + geom_text(aes(label=txt,x=x, y=y, hjust = 0),
                                   data=txt.all, size=3)
  }
    p <- p + theme(axis.text.x=theme_text(angle=-45))
